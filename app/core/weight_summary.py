...
def prepare_weight_series(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out[DATE_COL] = normalize_datetime_series(out[DATE_COL], dayfirst=True, normalize_day=False)
...
def detect_stagnation_periods(df: pd.DataFrame, window_days: int = 14, tolerance_kg: float | None = None) -> list[dict[str, Any]]:
    """Detect historical stagnation windows using the central plateau engine."""
    config = StagnationConfig(window_days=window_days)
    if tolerance_kg is not None:
        config = StagnationConfig(window_days=window_days, max_amplitude_kg=tolerance_kg)
    data = prepare_weight_series(df)
    ...
    for idx in range(len(data)):
        while (data[DATE_COL].iloc[idx] - data[DATE_COL].iloc[start_idx]).total_seconds() / 86400 > window_days:
            start_idx += 1
        window = data.iloc[start_idx : idx + 1]
        result = evaluate_plateau_window(window.rename(columns={DATE_COL: "Date", WEIGHT_COL: "Poids (Kgs)"}), config)
        if result.get("is_plateau") and result.get("period_days", 0) >= window_days - 2:
            candidate = {
                "start": result["period_start"],
                "end": result["period_end"],
                "days": int(result["period_days"]),
                "amplitude": result["amplitude"],
                "measurements": result["nb_mesures"],
                "slope_kg_week": result["slope_kg_week"],
            }
